...
test('Verify date time displays correctly after clicking the Biz Dev Active link', async ({ page }) => {

  // Navigate to Biz Dev Active page
  await reportPanelPage.clickSidebarToggle();
  await reportPanelPage.clickBizDevActiveLink();

  await bizDevActivePage.waitForRefreshDateTimeToBeVisible();

  const isRefreshDateTimeDisplayed = await bizDevActivePage.isRefreshDateTimeDisplayed();
