...
const expandButton = branchActivePage.activeReportExpandButton;
...
await branchActivePage.clickActiveReportExpand();
...
const expandButton = branchActivePage.loanInvestorExpandButton;
await expect(expandButton).toBeVisible({
  timeout:30000
});
if (await expandButton.isVisible({
...
await branchActivePage.clickLoanInvestorExpand();
});
...
await expect(branchActivePage.activeReportExpandButton).toBeVisible({
  timeout:30000
});
await expect(branchActivePage.activeReportExpandButton).toBeEnabled({
  timeout:30000
});

await branchActivePage.clickActiveReportExpand();

const collapseButton = branchActivePage.activeReportCollapseButton;
...
const collapseButton = branchActivePage.loanInvestorCollapseButton;
await expect(collapseButton).toBeVisible({
  timeout:30000
});
if (await collapseButton.isVisible({
