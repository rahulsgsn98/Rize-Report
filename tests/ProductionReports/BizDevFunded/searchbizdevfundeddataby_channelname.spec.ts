...
channelNameSets.forEach((channelNameSet) => {
  ...
  test(`Search Biz-Dev Funded: ${channelNameSet.join(", ")}`, async ({ page }) => {
    test.slow();
  ...
  await appRoot.waitFor({ state: 'visible', timeout: 30000 });
    ...
    await bizDevFundedPage.channelNameSelection(channelNameSet);

    await bizDevFundedPage.clickSubmit();

   // await bizDevFundedPage.verifyChannelNameData(channelNameSet);
