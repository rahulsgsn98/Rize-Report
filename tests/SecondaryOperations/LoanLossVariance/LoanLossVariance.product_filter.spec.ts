...
productSets.forEach((productSet) => {
  ...
  test(`Search Loan Loss Variance by product: ${productSet.join(', ')}`, async ({ page }) => {
    test.slow();
    ...
    await loanLossVariancePage.productSelection(productSet);

    await loanLossVariancePage.clickSubmit();

    await loanLossVariancePage.verifyProductData(productSet);
  });
