...
const branchSets = [
  ['Kendall Rize'],
  ['Pembroke Pines Rize', 'Menlo Park Rize'],
];

branchSets.forEach((branchSet) => {

  test(`Search Loan Loss Variance by branch: ${branchSet.join(', ')}`, async ({ page }) => {
    test.slow();
    ...
    await loanLossVariancePage.branchSelection(branchSet);

    await loanLossVariancePage.clickSubmit();

    await loanLossVariancePage.verifyBranchData(branchSet);
  });
