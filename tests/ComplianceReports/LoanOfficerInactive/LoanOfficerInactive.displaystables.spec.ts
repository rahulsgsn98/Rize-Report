...
test('Verify that the Compliance Report table displays correctly after click on the Loan Officer Inactive link', async ({ page }) => {

  await reportPanelPage.clickSidebarToggle();
  await reportPanelPage.clickLoanOfficerInactiveLink();

  const isVisible = await loanOfficerInactivePage.isComplianceReportTableVisible();
...
test('Verify that the Details table displays correctly after click on the Loan Officer Inactive link', async ({ page }) => {

  await reportPanelPage.clickSidebarToggle();
  await reportPanelPage.clickLoanOfficerInactiveLink();

  const isVisible = await loanOfficerInactivePage.isDetailsTableVisible();
