...
test('Verify date time displays correctly after click on the Verbal VOE link', async ({ page }) => {

  await reportPanelPage.clickSidebarToggle();
  await reportPanelPage.clickVerbalVOELink();

  await verbalVOEPage.waitForRefreshDateTimeToBeVisible();

  const isRefreshDateTimeDisplayed = await verbalVOEPage.isRefreshDateTimeDisplayed();
