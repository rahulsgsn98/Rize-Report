...
test('Verify that the Details button is visible and clickable - Mobile & Tablet Only', async ({ page, isMobile }) => {
  ...
  const detailsButton = verbalVOEPage.mobileDetailsButtons.first();
  ...
  await verbalVOEPage.clickFirstMobileDetailsButton();
});
...
test('Verify that the Details modal opens and closes correctly - Mobile & Tablet Only', async ({ page, isMobile }) => {
  ...
  const isModalVisible = await verbalVOEPage.isMobileDetailsModalVisible();
  ...
  // Verify modal title
  const modalTitle = verbalVOEPage.page.locator('h4.popup-title');
  await expect(modalTitle).toBeVisible({ timeout: 30000 });
  await expect(modalTitle).toHaveText(/Details/i);

  // Verify close button is visible
  const closeButton = verbalVOEPage.mobileDetailsModalCloseButton;
  await expect(closeButton).toBeVisible({ timeout: 30000 });
  await expect(closeButton).toBeEnabled({ timeout: 30000 });

  await verbalVOEPage.closeMobileDetailsModal();

  await expect(verbalVOEPage.mobileDetailsModal).toBeHidden({ timeout: 30000 });
