...
test('Verify that the Compliance Report Expand button is visible and clickable', async ({ page }) => {
  ...
  const expandButton = verbalVOEPage.compliancereportexpandButton;
  ...
  await verbalVOEPage.clickComplianceReportExpand();
});
...
test('Verify that the Compliance Report Collapse button is visible and clickable after expanding', async ({ page }) => {
  ...
  await expect(verbalVOEPage.compliancereportexpandButton).toBeVisible({ timeout: 30000 });
  await expect(verbalVOEPage.compliancereportexpandButton).toBeEnabled({ timeout: 30000 });

  await verbalVOEPage.clickComplianceReportExpand();

  const collapseButton = verbalVOEPage.compliancereportcollapseButton;
