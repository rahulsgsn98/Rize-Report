...
test('Verify that the Compliance Report table displays correctly after click on the Shipping link', async ({ page }) => {

  await reportPanelPage.clickSidebarToggle();
  await reportPanelPage.clickShippingLink();

  const isVisible = await shippingReportPage.iscomplianceReportTable()

  expect(isVisible).toBeTruthy();

  console.log('Shipping Compliance Report table displays', isVisible);
});