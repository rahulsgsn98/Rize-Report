...
async waitForGeographicLoadingToFinish(timeout = 90000) {
  ...
  // await this.waitForCountyMapToRender()
    await Promise.race([
      this.activeLoanDesktopRefresh.waitFor({ state: "visible", timeout: 30000}),
      this.activeLoanMobileRefresh.waitFor({ state: "visible", timeout: 30000})
    ]);
  }

  async isRefreshDateTimeDisplayed(): Promise<boolean> {
    return (
      await this.activeLoanDesktopRefresh.isVisible(
        {
          timeout:30000
        }
      ).catch(() => false)
    ) || (
      await this.activeLoanMobileRefresh.isVisible(
        {
          timeout:30000
        }
      ).catch(() => false)
  ...
  async isLoanStatusChartVisible(): Promise<boolean> {
    ...
    await expect(this.activeLoanStatusChart).toBeVisible(
      {
          timeout:30000
        }
    );
  ...
  async isLoanChannelChartVisible(): Promise<boolean> {
    ...
    await expect(this.activeLoanChannelChart).toBeVisible(
     {
          timeout:30000
        }
    );
  ...
  async isProductNameChartVisible(): Promise<boolean> {
    ...
    await expect(this.activeProductNameChart).toBeVisible(
     {
          timeout:30000
        }
    );
  ...
  async isLoanPurposeChartVisible(): Promise<boolean> {
    ...
    await expect(this.activeLoanPurposeChart).toBeVisible(
     {
          timeout:30000
        }
    );
  ...
  async isLoanInvestorChartVisible(): Promise<boolean> {
    ...
    await expect(this.activeLoanInvestorChart).toBeVisible(
    {
          timeout:30000
        }
    );
  ...
  async isLoanChannelReportTableVisible(): Promise<boolean> {
    ...
    await expect(this.activeLoanChannelReportTable).toBeVisible(
      { timeout: 30000}
    );
  ...
  async isProductNameReportTableVisible(): Promise<boolean> {
    ...
    await expect(this.activeProductNameReportTable).toBeVisible
    (
      { timeout: 30000}
  ...
  async isLoanPurposeReportTableVisible(): Promise<boolean> {
    ...
    await expect(this.activeLoanPurposeReportTable).toBeVisible(
      { timeout: 30000}
    );
  ...
  async isLoanInvestorReportTableVisible(): Promise<boolean> {
    ...
    await expect(this.activeLoanInvestorReportTable).toBeVisible(
     { timeout: 30000}
    );
 ...
 */
  ...
  // await this.waitForGeographicLoadingToFinish()
  await this.waitForCountyMapToRender()
